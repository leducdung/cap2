...
import { StoreOwners, fieldNeedToUseRegex } from './model/storeOwners.interface';
...
export class StoreOwnersService {
  ...
  async findMany({ query }) {
    try {
      ...
      if (hasPage) {
        promises.push(
          this.storeOwnersModel.find(findingQuery)
            .populate('createdBy')
            .sort(sortingCondition)
            .limit(limits)
            .skip(skip)
            .exec(),
          this.storeOwnersModel.countDocuments(findAllQuery)
            .exec(),
        )
      }
