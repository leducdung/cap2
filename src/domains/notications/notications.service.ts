...
export class NoticationsService {
 ...
 async findOne({data}: any): Promise<Notications | string> {
    try {
      ...
      const notication = await this.noticationsModel.findOne(data).populate('storeOwnerID').exec()
  ...
  async findProduct(query): Promise<Notications | string> {
    try {
      ...
      const notication = await this.noticationsModel.findOne(query).populate('storeOwnerID').exec()
