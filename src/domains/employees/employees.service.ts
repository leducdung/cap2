...
export class EmployeesService {
 ...
 async findOne({data}: any): Promise<Employees | string> {
    try {
      ...
      const employees = await this.employeesModel.findOne(data).populate('storeOwnerID').exec()
  ...
  async findOneStoreOwner(query): Promise<Employees | string> {
    try {
      const employees = await this.employeesModel.findOne(query).populate('storeOwnerID').exec()
  ...
  async deleteOne( query ): Promise<boolean | string> {
    try {
      const employees = await this.employeesModel.findOne(query)
      .exec()
  ...
  async findMany({ query }) {
    try {
      ...
      if (hasPage) {
        promises.push(
          this.employeesModel.find(findingQuery)
            .populate('storeOwnerID')
            .sort(sortingCondition)
            .limit(limits)
            .skip(skip)
            .exec(),
          this.employeesModel.countDocuments(findAllQuery)
            .exec(),
        )
      }
