...
import { CommentAndRatings, fieldNeedToUseRegex } from './model/commentAndRatings.interface';
...
export class CommentAndRatingsService {
 ...
 async findOne({data}: any): Promise<CommentAndRatings | string> {
    try {
      ...
      const commentAndRating = await this.commentAndRatingModel.findOne(data).populate('storeOwnerID').exec()
  ...
  async deleteOne( query ): Promise<boolean | string> {
    try {
      const commentAndRating = await this.commentAndRatingModel.findOne(query)
      .exec()
  ...
  async findMany({ query , productID}) {
    try {
      ...
      if (hasPage) {
        promises.push(
          this.commentAndRatingModel.find(findingQuery)
            .populate('storeOwnerID')
            .sort(sortingCondition)
            .limit(limits)
            .skip(skip)
            .exec(),
          this.commentAndRatingModel.countDocuments(findAllQuery)
            .exec(),
        )
      }
